"""
The code is taken from: https://github.com/martinagvilas/vit-cls_emb/tree/main/src
The authors of this paper acknowledge the original work and thanks the authors for releasing the code.
"""
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.perturb == 'self_only':
            for t in range(1, attn.shape[-1]):
                attn[:, :, t, :] = 0
                attn[:, :, t, t] = 1 
        elif self.perturb == 'no_cls':
            for t in range(1, attn.shape[-1]):
                attn[:, :, t, 0] = 0

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        self.key_proj_vals = x
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = Attention(
            ...
            proj_drop=drop, perturb=perturb_type
        )
    ...
    def forward(self, x):
        attn_x = self.ls1(self.attn(self.norm1(x)))
        self.attn_cls = self.func_cls(self.norm_cls(attn_x))
        #self.attn_cls = self.func_cls(self.norm_cls(self.attn(self.norm1(x))))
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, tokens_mask=None):
    ...
    def forward(self, x, tokens_mask=None):
        x = self.forward_features(x, tokens_mask)
